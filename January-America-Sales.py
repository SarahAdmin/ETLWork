...
columnstoRemove = ['Applied On', 'Collected On', 'Anonymized Contact ID', 'Posting ID','Commitment / Work Type',
                   'Origin',
       'Sources', 'Current Stage', 'Current Stage ID',
       'Application Archive Reason', 'Application Archived On',
       'Application Archived By', 'Survey Id', 'Survey Name',
       'Survey Location Type', 'Posting Title', 'Posting Owner', 'Candidate Selected Location',
...
new_JanDF = JanDF.drop(columnstoRemove, axis=1)
JanSales = new_JanDF[(JanDF['Department'] == 'Sales') & (JanDF['Location'] == 'USA (Remote)')].dropna()
print(JanSales.to_string())
